...
from lisa.tools import Cat, Ethtool, Iperf3, Lsmod, Lspci, Modprobe, Service, Ssh
...
class Sriov(TestSuite):
    ...
    #  unload/load mlx4_en will unload/load mlx4_core
    # ConnectX-4/ConnectX-5 uses mlx5_core
    #  unload/load mlx5_ib will unload/load mlx5_core
    ...
    def sriov_ethtool_offload_setting(self, environment: Environment) -> None:
        ...
        dest_iperf3.run_as_client(
            server_node.internal_address, client_iperf3_log, 1800, True
        )
        ...
        dest_cat = client_node.tools[Cat]
        iperf_log = dest_cat.read_from_file(
            "iperfResults.log", sudo=True, force_run=True
        )
        assert_that(iperf_log).does_not_contain("error")
    ...
    def _stop_firewall(self, environment: Environment) -> None:
        for node in environment.nodes.list():
            ...
            node.execute("command -v ufw && ufw disable", shell=True, sudo=True)
            node.execute(
                "command -v iptables && iptables -P INPUT ACCEPT &&"
